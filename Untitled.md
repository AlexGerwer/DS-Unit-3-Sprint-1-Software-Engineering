...
I find that looking for accidental errors (e.g., typos) as well as structural
errors (e.g., dead code, logic or algorithm bugs, performance or architecture
concerns) is important because often such errors are much easier to spot for
critical reviewers with an outside perspective.  I think that it is also
important to ensure that refactoring changes do not alter code behavior and,
conversely, that code behavior-changing alterations avoid refactoring and code
formatting changes.  An outside reviewer also is able to point out aspects of
documentation which are not clear.  I have learned how to make my code more
efficient through review.  Finally, I am a strong believer in proper naming.
I have often heard from less experienced software engineers that the naming of
variables, functions and classes are not very important. This cannot be further
from the truth, though. Code is written for two audiences: a machine and a
human. The machine does not care about names, in truth it ignores them almost
all together. The human, on the other hand, cares deeply about names.
Meaningful names turn confusing code into something that makes sense in the
...
Containers help with making applications hardware independent and interoperable.  
Containers increase predictability and dependability by providing each application
container with its own separate environment containing the application itself as
well as everything that application needs to run.  Containers also increase speed
from development to deployment by removing technical dependencies that are either
overlooked or not fully understood in the race to create a new application.
Finally, containers increase operational velocity by requiring developers to
to update one things at a time without worrying about how such updates will affect
other processes.

In summary, the following are key advantages of containerization:
  Enhanced collaboration
  Greater application portability
  Faster application deployment
  Improved efficiency in development
  Increased ease of application scaling
